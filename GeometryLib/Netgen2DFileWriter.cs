...
public class Netgen2DFileWriter : GeometryWriter
{
    public Netgen2DFileWriter(Geometry geometry) : base(geometry)
    {
    }
    ...
    public override void Write(string filePath)
    {
        ...
        // Emit points
        foreach (var pt in base.geometry.Points.OrderBy(p => p.Id))
        {
            writer.WriteLine($"point {pt.Id} {pt.x} {pt.y}");
        }

        // Emit lines
        foreach (var line in base.geometry.Lines.OrderBy(l => l.Id))
        {
            writer.WriteLine($"line {line.Id} {line.pt1.Id} {line.pt2.Id}");
        }

        // Emit arcs
        foreach (var arc in base.geometry.Arcs.OrderBy(a => a.Id))
        {
        ...
        // Emit line loops
        foreach (var loop in base.geometry.LineLoops.OrderBy(l => l.Id))
        {
        ...
        // Emit surfaces
        foreach (var surface in base.geometry.Surfaces.OrderBy(s => s.Id))
        {
